...
/**
 * All scroll- and pointer-driven motion for the marketing page.
 *
 ...
 *
 * Interactive widgets (theme switch, drawer, FAQ, demo form) are React state —
...
export function useLandingMotion(rootRef: RefObject<HTMLDivElement | null>) {
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      if (!reduce) {
        const STAGGER = [".grid4", ".tiers", ".metrics", ".gallery"];
      ...
      /* ---------- 6. pinned horizontal stories ---------- */
      if (!reduce) {
        const sec = root.querySelector<HTMLElement>(".stories");
        ...
        if (sec && track) {
          ...
          gsap.set(track, { perspective: 1500 });
          gsap.utils.toArray<HTMLElement>(".story").forEach((s) => {
            gsap.from(s, {
              rotateY: 12,
              scale: 0.95,
              opacity: 0.5,
