 ...
 *      characters are safe.
 */
export function buildDatabaseUrl(source: Record<string, string | undefined> = process.env): string {
...
const serverSchema = z.object({
  ...
  /* Optional: defaults to localhost on whatever PORT is set to, so changing the
     port in .env is a single edit. Set it explicitly for any real deployment —
     behind a proxy or a domain the app cannot infer its own public URL. */
...
/** Same shape, but the app URL is always resolved. */
export type Env = ParsedEnv & { NEXT_PUBLIC_APP_URL: string };
...
function loadEnv(): Env {
  ...
  if (typeof window !== "undefined" || process.env.SKIP_ENV_VALIDATION) {
    // same fallback applies when validation is skipped, so callers never see undefined
    const unvalidated = raw as unknown as ParsedEnv;
    return withDefaults({ ...unvalidated, PORT: Number(unvalidated.PORT) || 3000 });
  }
...
/** One place to derive values from other values. Changing PORT in .env is enough
 *  for local work; a deployment still sets NEXT_PUBLIC_APP_URL explicitly. */
function withDefaults(parsed: ParsedEnv): Env {
  return {
    ...parsed,
    NEXT_PUBLIC_APP_URL: parsed.NEXT_PUBLIC_APP_URL ?? `http://localhost:${parsed.PORT}`,
  };
}
